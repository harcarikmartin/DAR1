				...
				<div class="col-lg-3 col-md-3 col-sm-3 col-xs-5">
					<c:if test="${sessionScope.user != null}">
						<form method="post">
							...
							<button class="btn-block-right addTaskBtn" type="submit">Add new task</button>
						</form>
					</c:if>
					<c:if test="${sessionScope.user == null}">
						<form method="post">
							<input type="hidden" name="action" value="addTask">
							<button class="btn-block-right addTaskBtn" disabled="disabled" type="submit">Add new task</button>
						</form>
					</c:if>
				</div>
			</div>
							...
							<c:forEach items="${topicTasks}" var="topicTask">
								<tr class="commentTR">
									...
									<td class="actionCell text-center taskNoButton">
										<c:if test="${topicTask.user.userID == sessionScope.user.userID || sessionScope.user.role == 'admin'}">
											<form method="post">
